...
const toTitleCase = (arr) => {
  const regex = /^[a-z]/;
  return arr.map((word) => {
    return word.replace(regex, word[0].toUpperCase());
  });
  // Solution code here...
};
...
let biggerThanLuke = (arr) => {
  ...
  const biggerThanLuke = arr.filter((character) => character.mass > lukeMass);
  const names = biggerThanLuke.map((character) => character.name);
  return names.join(' - ');
  // Solution code here...
};
...
const detectTicTacToeWin = (board) => {
  ...
  const x = arr.filter((letter) => regex.test(letter));
  const o = arr.filter((letter) => !regex.test(letter));

...
const firstLetters = (arr) => {
  const firstLetter = arr.map((value) => {
    return value[0];
...
const onlyOddChars = (str) => {
  const odd = str.split('').filter((value, index) => {
    return index % 2 !== 0;
  });
  return odd.join('');
};
